...
export default {
    ...
    methods: {
        vote(type) {
            if(! __auth()) {
                return this.$toast.error('Please login to vote this video', 'Error', { timeout:3000 });
            }

            if(__auth().id === this.entity_owner) {
                return this.$toast.warning('You cannot like your own video', 'Warning', { timeout:3000 });
            }
